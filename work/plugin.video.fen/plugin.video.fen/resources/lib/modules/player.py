...
class FenPlayer(xbmc_player):
	...
	def play_video(self, url, obj):
		...
		logger('FEN', 'DEBUG: play_video iniciado')
	...
	def monitor(self):
		try:
			from modules.kodi_utils import logger
			logger('FEN', 'DEBUG: Método monitor() iniciado')
			...
			hide_busy_dialog()
			from modules.kodi_utils import logger
			logger('FEN', 'DEBUG: Total check time: %s, Fullscreen: %s' % (total_check_time, get_visibility(video_fullscreen_check)))
			logger('FEN', 'DEBUG: Entrando no loop de monitorização')
			sleep(1000)
			while self.isPlayingVideo():
				try:
					...
					if not self.subs_searched: 
						from modules.kodi_utils import logger
						logger('FEN', 'DEBUG: Chamando run_subtitles')
						self.run_subtitles()
				except: pass
			hide_busy_dialog()
			if not self.media_marked: self.media_watched_marker()
	...
	def run_subtitles(self):
		...
		from modules.kodi_utils import logger
		logger('FEN', 'DEBUG: run_subtitles chamado - title=%s, imdb=%s' % (self.title[:30] if self.title else "None", self.imdb_id))
		try: Thread(target=Subtitles().get, args=(self.title, self.imdb_id, self.season or None, self.episode or None)).start()
		except Exception as e: 
			logger('FEN', 'DEBUG: Erro em run_subtitles: %s' % str(e))
		
	def show_subtitle_overlay(self):
		"""Show subtitle selection overlay during playback"""
		try:
			subtitle_manager = Subtitles()
			subtitle_manager.show_subtitle_selection_overlay(self.title, self.imdb_id, self.season or None, self.episode or None)
		except: pass
	...
	def set_playback_properties(self):
		try:
			...
			# Set properties for subtitle overlay access
			set_property('fen.current_title', self.title)
			set_property('fen.current_imdb_id', self.imdb_id)
			set_property('fen.current_season', str(self.season) if self.season else '')
			set_property('fen.current_episode', str(self.episode) if self.episode else '')
		except: pass
	...
	def clear_playback_properties(self):
		...
		# Clear subtitle overlay properties  
		clear_property('fen.current_title')
		clear_property('fen.current_imdb_id')
		clear_property('fen.current_season')
		clear_property('fen.current_episode')
...
class Subtitles(xbmc_player):
	...
	def get(self, query, imdb_id, season, episode, secondary_search=False):
		...
		# Debug dos parâmetros de entrada
		from modules.kodi_utils import logger
		logger('FEN', 'DEBUG: query=%s, imdb=%s, lang=%s' % (query[:30] if query else "None", imdb_id, self.language))
	...
	def _sort_subtitles(self, subtitles_list):
		"""Sort subtitles by preference (language match, then quality)"""
		if not subtitles_list:
			return []
			
		def subtitle_score(sub):
			score = 0
			release_name = sub.get('MovieReleaseName', '').lower()
			
			# Language preference
			if sub.get('SubLanguageID') == self.language:
				score += 100
				
			# Quality preference (higher quality = higher score)
			quality_scores = {
				'bluray': 50, 'brrip': 45, 'bdrip': 45,
				'webdl': 40, 'web-dl': 40, 'webrip': 35,
				'hdrip': 30, 'hdtv': 25, 'dvdrip': 20
			}
			
			for quality, points in quality_scores.items():
				if quality in release_name:
					score += points
					break
					
			# Resolution preference
			if '1080p' in release_name: score += 15
			elif '720p' in release_name: score += 10
			elif '2160p' in release_name or '4k' in release_name: score += 20
			
			return score
			
		return sorted(subtitles_list, key=subtitle_score, reverse=True)
		
	def _auto_select_subtitle(self, subtitles_list, query, video_path, season=None, episode=None):
		"""Improved automatic subtitle selection with multiple strategies"""
		if not subtitles_list:
			return None
		...
		video_name_clean = clean_text(os.path.splitext(video_path)[0]) if video_path else ""
		query_keywords = extract_keywords(query)
		video_keywords = extract_keywords(video_path) if video_path else []
		
		# Strategy 1: Exact title match (highest priority)
		for sub in subtitles_list:
			if sub['SubLanguageID'] == self.language:
				release_name_clean = clean_text(sub.get('MovieReleaseName', ''))
				if query_clean in release_name_clean or any(keyword in release_name_clean for keyword in query_keywords):
					return sub
		
		# Strategy 2: Video filename match with preferred language
		if video_name_clean:
			for sub in subtitles_list:
				if sub['SubLanguageID'] == self.language:
					release_name_clean = clean_text(sub.get('MovieReleaseName', ''))
					if video_name_clean in release_name_clean or any(keyword in release_name_clean for keyword in video_keywords):
						return sub
		
		# Strategy 3: Quality + Language match
		if video_path:
			video_lower = video_path.lower()
			quality_found = None
			for quality in self.quality:
				if quality in video_lower:
			...
			if quality_found:
				for sub in subtitles_list:
					if sub['SubLanguageID'] == self.language:
						release_lower = sub.get('MovieReleaseName', '').lower()
						if quality_found in release_lower:
							return sub
		
		# Strategy 4: Season/Episode match for TV shows
		if season and episode:
			season_patterns = [f's{season:02d}e{episode:02d}', f'{season}x{episode:02d}', f'season.{season}']
		...
		# Strategy 5: Keyword scoring for best match
		best_sub = None
		best_score = 0
		...
		# Strategy 6: Just get the best by language (fallback)
		lang_matches = [i for i in subtitles_list if i['SubLanguageID'] == self.language]
		if lang_matches:
			return lang_matches[0]
		
		# Strategy 7: Last resort - any subtitle
		return subtitles_list[0] if subtitles_list else None
		
	def _find_perfect_match(self, subtitles_list, query, video_path, season=None, episode=None):
		"""Find perfect match for automatic selection (strict criteria)"""
		if not subtitles_list:
			return None
		...
		query_clean = clean_for_match(query)
		video_clean = clean_for_match(os.path.splitext(video_path)[0]) if video_path else ""
		
		# Strategy 1: Exact title match with preferred language
		for sub in subtitles_list:
			if sub['SubLanguageID'] == self.language:
				release_clean = clean_for_match(sub.get('MovieReleaseName', ''))
				
				# Perfect title match
				if query_clean == release_clean or query_clean in release_clean:
					return sub
				
				# Perfect video filename match (without quality tags)
				if video_clean and video_clean in release_clean:
					return sub
		
		# Strategy 2: Season/Episode exact match for TV shows
		if season and episode:
			patterns = [
			...
			for sub in subtitles_list:
				if sub['SubLanguageID'] == self.language:
					...
					# Check if both title and episode pattern match
					if any(pattern in release_lower for pattern in patterns):
						# Also check if title keywords are present
						title_words = query_clean.split()
						if any(word in release_lower for word in title_words if len(word) > 3):
							return sub
		
		return None  # No perfect match found
		
	def _search_clean_title(self, query, imdb_id, season=None, episode=None):
		"""Search again with clean title only (no quality tags, users, etc.)"""
		try:
			# Clean the query - remove common quality/release tags
			clean_query = self._clean_title_for_search(query)
			
			# Don't search again if the query is essentially the same
			if clean_query.lower() == query.lower():
				return None
			
			# Search with clean title
			result = self.os.search(clean_query, imdb_id, self.language, season, episode)
			if result and len(result) > 0:
				# Filter for single CD
				filtered = [i for i in result if i['SubSumCD'] == '1']
	...
	def _clean_title_for_search(self, title):
		"""Clean title by removing quality tags, user names, etc."""
		# Remove common quality indicators
		quality_tags = ['webrip', 'bluray', 'brrip', 'bdrip', 'dvdrip', 'webdl', 'web-dl', 
						'hdrip', 'hdtv', 'cam', 'ts', 'tc', 'telesync', 'dvdscr', 'screener']
		
		# Remove resolution tags
		resolution_tags = ['1080p', '720p', '480p', '2160p', '4k', 'uhd']
		
		# Remove codec tags
		codec_tags = ['x264', 'x265', 'h264', 'h265', 'xvid', 'divx', 'avc', 'hevc']
		
		# Remove common release group patterns (usually in brackets or after dashes)
		title_clean = title
		
		# Remove bracketed content [like this]
		title_clean = re.sub(r'\[.*?\]', '', title_clean)
		
		# Remove parentheses content (except years)
		title_clean = re.sub(r'\((?!(?:19|20)\d{2}\)).*?\)', '', title_clean)
		
		# Convert to lowercase for tag removal
		title_lower = title_clean.lower()
		
		# Remove quality tags
		for tag in quality_tags + resolution_tags + codec_tags:
			title_lower = re.sub(r'\b' + tag + r'\b', '', title_lower)
		...
		return title_lower.title() if title_lower else title
		
	def show_subtitle_selection_overlay(self, query=None, imdb_id=None, season=None, episode=None):
		"""Public method to show subtitle overlay during playback"""
		if not query or not imdb_id:
			notification("Informações insuficientes para buscar legendas", 3000)
			return
		...
		try:
			...
			result = self.os.search(query, imdb_id, self.language, season, episode)
			if not result or len(result) == 0:
				notification("Nenhuma legenda encontrada", 3000)
				return
				
			# Filter for single CD subtitles
			filtered_result = [i for i in result if i['SubSumCD'] == '1']
			if not filtered_result:
				filtered_result = result
				
			# Show overlay
			chosen_sub = self._show_subtitle_overlay(filtered_result)
			if chosen_sub:
				# Download and set the selected subtitle
				try:
					lang = convert_language(chosen_sub['SubLanguageID'])
				except:
				...
				subtitle_file = self.os.download(download_url, subtitle_path, temp_zip, temp_path, final_path)
				if subtitle_file:
					self.setSubtitles(subtitle_file)
					if self.auto_enable == 'true':
						self.showSubtitles(True)
					notification("Legenda carregada com sucesso", 2000)
				else:
					notification("Erro ao baixar legenda", 3000)
		except Exception as e:
			notification("Erro ao carregar legendas", 3000)
