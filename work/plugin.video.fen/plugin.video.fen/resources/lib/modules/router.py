...
def routing(sys):
	...
	if mode == 'update_check':
		...
		return update_check()
	if mode == 'show_subtitle_overlay':
		from modules.kodi_utils import get_property, notification
		# Get current playback info from player properties
		title = get_property('fen.current_title')
		imdb_id = get_property('fen.current_imdb_id') 
		season = get_property('fen.current_season')
		episode = get_property('fen.current_episode')
		
		notification("Title: %s, IMDB: %s" % (title or "None", imdb_id or "None"), 3000)
		
		if title and imdb_id:
			from modules.player import Subtitles
			...
			return subtitle_manager.show_subtitle_selection_overlay(title, imdb_id, season, episode)
		else:
			return notification("Nenhum conteúdo em reprodução", 3000)
